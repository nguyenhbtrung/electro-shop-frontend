...
import { GetAllAttribute, CreateAttributeDetail, DeleteAttribute } from "../../../services/attributeService";
...
const ManageAttribute = () => {
  ...
  const handleDelete = async (row) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa thuộc tính này không?")) {
      try {
        await DeleteAttribute(row.attributeId);
    ...
    <Box m="20px">
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
        <Header
          ...
          subtitle="Danh sách các thuộc tính sản phẩm"
        />
